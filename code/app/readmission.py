...
def get_monthly_avg_admission_(df):
    output = df.groupby(by=[df['Date'].dt.month])['percentage_admission'].mean().reset_index()
    return output
...
def readmission_compare_line():
    ...
    fig.update_layout(title=dict(text='Comparision to National and State Rates',
                                 x=0.5,
                                 xanchor='center',   
                                 font=dict(size=22)        
                                ),
...
def service_pie():
    ...
    fig.update_traces(textposition='inside', 
                    ...
                    textfont_size=16,
                    marker=dict(colors=colors, line=dict(color='#e5e5e5', width=0.9)))
