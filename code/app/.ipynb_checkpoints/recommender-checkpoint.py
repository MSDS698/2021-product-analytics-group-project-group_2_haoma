...
class Recommender():
    ...
    def __init__(self):
        ...
        # Hyperparameters
        self.tier_weights = {'star': 0, 'flagged': 2, 'ppr': 2, 'dtc': 1} 
        self.num_agencies = 10

        
    def filter_zipcode(self, zipcode):
        cms_nums = self.df_zip[self.df_zip[' ZIP Code'] == zipcode]['CMS Certification Number (CCN)']
        df_filter = self.df_cal[self.df_cal['CMS Certification Number (CCN)'].isin(list(cms_nums))]
        return df_filter
    ...
    def recommend(self, df_filter, bool_services, pdf_path):
        ...
        pipe_prep = Pipeline([('Drop Unnecessary Columns', Drop()),
                              ...
                              ('Recommend', Recommend(self.tier_weights, flagged_qtopic, self.num_agencies))])
